---
title: "Question1_Evaluation_905"
...
Après avoir filtré pour ne conserver que les échantillons  relatif à l'espèce Charme et intégré l'effet du relevé dans le modèle, il ne reste plus de variable susceptible de créer une dépendance dans les résidus, excepté peut-être la position précise des arbres au sein des relevés ou peut-être d'autres variabe en lien avec la qualité du sol ou la présence de l'eau mais on dispose pas ces informations dans le jeu de données étudié.
 Mais ça n'empêche pa d'appliquer un Test comme le Testde Durbin-Watsonest utilisé pour détecter la présence d'autocorrélation des résidus.
...
# Si vous voulez revenir à l'échelle originale, on applique la transformation inverse :
IC_original <- ((modAnova_pT$lambda * IC_transformed) + 1)^(1 /modAnova_pT$lambda)
print(IC_original)
```

 ***Visualisation graphique de ce résultats 
```{r}
...
𝐹=207.726 : Cette valeur de F test est très élevée. Cela suggère une forte variation expliquée par les groupes(les rélévées) dans le modèle par rapport à la variation non expliquée (résiduelle). En effet, plus F est grand, plus il est probable que les différences observées entre les groupes soient significatives.
